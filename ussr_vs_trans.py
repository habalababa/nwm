...
pygame.display.set_caption("ussr vs trans")
...
bulletshotsnd = pygame.mixer.Sound(os.path.join("gra", "bulshotbb.mp3"))
winsnd = pygame.mixer.Sound(os.path.join("gra", "winnnc.mp3"))

border = pygame.Rect(width//2 - 2.5, 0, 5, height)

redshipjpg = pygame.image.load(os.path.join("gra", "statekussr.png"))
redship = pygame.transform.rotate(pygame.transform.scale(redshipjpg, (shiplng, shiphgt)), 0)

blueshipjpg = pygame.image.load(os.path.join("gra", "statektrans.png"))
...
def  upd(red, blue, redbullets, bluebullets, redhp, bluehp):
    ...
    redhptxt = hpfont.render("health: " + str(bluehp), True, redcl)
    bluehptxt = hpfont.render("health: " + str(redhp), True, redcl)
...
def handlebullets(bluebullets, redbullets, red, blue):
    ...
    for bullet in redbullets:
        ...
        if red.colliderect(bullet):
            ...
            redbullets.remove(bullet)
def winneris(text):
    istxt = winfont.render(text, True, brd)
